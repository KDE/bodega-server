...
DECLARE
    tagId int;
BEGIN
    SELECT INTO tagID t.id from tags t INNER JOIN tagtypes ttype ON (t.type = ttype.id) WHERE ttype.type = 'assetType' AND t.id = NEW.tag;
    IF NOT FOUND THEN
...
CREATE TRIGGER trg_ct_checkTagForRating BEFORE INSERT OR UPDATE ON ratings
