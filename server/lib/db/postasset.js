...
function hasIcons(db, req, res, assetInfo)
{
    ...
    for (i = 0; i < keys.length; ++i) {
        previewInfo = assetInfo.previews[i];
        preview = req.files[previewInfo.file];
        if (preview) {
            foundPreview = true;
        }
    }
...
function hasPreviews(db, req, res, assetInfo)
{
    var keys = Object.keys(assetInfo.previews);
    var i;
    var previewInfo, preview;
...
function publishAsset(db, req, res, assetInfo, cb)
{
    cb(null, db, req, res, assetInfo);
}
