    ...
    task.db.query("select case WHEN p.service IS NULL THEN '' ELSE p.service END,\
             ...
             function(err, result) {
                 ...
                 task.db.query("select 'assets' as type, count(id)::int as count from assets where partner = $1 \
                                ...
                                union \
                                select 'storeassets' as type, count(distinct ca.asset)::int from subchannelassets ca \
                                        join channels c ON (ca.channel = c.id) \
                                        WHERE c.parent IS NULL AND \
                                              c.store IN (SELECT id FROM stores WHERE partner = $1) \
                                order by type;",
                                [ task.partner ],
                                function(err, result) {
                                    if (err) {
...
{
    db.query("INSERT INTO affiliations (person, partner, role) SELECT $1, $2, id FROM personRoles",
